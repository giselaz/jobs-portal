        <x-layouts.landing>
            <x-ui.hero-section>
                <x-ui.search-bar class="mt-6" :action="route('jobs.index')" />
                <p class="mt-4 text-sm font-medium text-slate-500">Popular Search</p>
                <div class="mt-2 flex flex-wrap gap-2">
                    <x-ui.tag variant="pill">UX/UI</x-ui.tag>
                    <x-ui.tag variant="pill">Web Development</x-ui.tag>
                    <x-ui.tag variant="pill">Human Resources</x-ui.tag>
                </div>
                </x-hero-section>

                <section class="border-t border-slate-100 bg-white py-16">
                    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                        <x-ui.section-title title="Job Category"
                            subtitle="Get the most exciting jobs and grow your career fast with others" class="mb-10" />
                        <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                            <x-ui.category-card title="UX/UI Designer" :open-positions="50" :active="true">
                                <x-heroicon-o-photo class="h-6 w-6" />
                            </x-ui.category-card>
                            <x-ui.category-card title="Website Development" :open-positions="120">
                                <x-heroicon-o-code-bracket class="h-6 w-6" />
                            </x-ui.category-card>
                            <x-ui.category-card title="Digital Marketing" :open-positions="78">
                ...
                <section class="border-t border-slate-100 bg-slate-50/30 py-16">
                    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                        <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                            <x-ui.section-title title="Popular Jobs"
                                subtitle="Check out featured jobs from top companies around the globe and apply now" />
                            <a href="{{ route('jobs.index') }}" class="shrink-0">
                                <x-ui.button variant="primary">See More →</x-ui.button>
                            </a>
                        </div>
                        <div class="mt-10 space-y-4">

                            @foreach ($popularJobs as $job)
                                <x-ui.job-listing-card :title="$job->title" :location="$job->location" :applyUrl="route('candidate.job.application.create', $job)">
                                    <div class="flex gap-2">
                                        <x-ui.tag variant="pill">
                                ...
                                </x-ui.job-listing-card>
                            @endforeach
                        </div>
                    </div>
                </section>

                <x-ui.company-logo-group>
