...
@php
    $base =
        'inline-flex items-center justify-center rounded-sm px-2.5 py-1.5 text-center text-sm font-semibold shadow-sm cursor-pointer transition focus:outline-none focus:ring-2 focus:ring-offset-2';
    if ($variant === 'primary' || $variant === 'outline') {
        ...
        $class =
            $base .
            ' border border-cyan-500 text-black ' .
            ($isLogout ? 'hover:text-red-500 hover:bg-transparent' : 'hover:text-white hover:bg-cyan-500');
    }
@endphp
@if ($href)
