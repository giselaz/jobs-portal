<x-layouts.landing>
    <section class="border-t border-slate-100 bg-slate-50/30 py-10">
        <div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
            <a href="{{ route('jobs.index') }}" class="mb-6 inline-flex items-center gap-1.5 text-sm font-medium text-slate-600 hover:text-violet-600 transition">
                ...
                <div class="border-b border-slate-100 bg-white p-6 sm:p-8">
                    <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                        <div class="flex gap-4">
                            <div class="flex h-14 w-14 shrink-0 items-center justify-center rounded-full bg-violet-100 text-lg font-semibold text-violet-700">
                                {{ strtoupper(mb_substr($job->employer->company_name, 0, 2)) }}
                            </div>
                            <div>
                                <h1 class="text-xl font-bold tracking-tight text-slate-900 sm:text-2xl">{{ $job->title }}</h1>
            ...
            @if ($otherJobs->isNotEmpty())
                <div class="mt-10">
                    <x-section-title
                        title="More jobs at {{ $job->employer->company_name }}"
                        class="mb-6 text-left"
                    />
                    <div class="space-y-3">
                        @foreach ($otherJobs as $otherJob)
                            <a href="{{ route('jobs.show', ['job' => $otherJob]) }}" class="block rounded-xl border border-slate-200 bg-white p-4 shadow-sm transition hover:border-violet-200 hover:shadow md:flex md:items-center md:justify-between md:gap-4">
                                <div>
                                    <h3 class="font-semibold text-slate-900 hover:text-violet-600 transition">{{ $otherJob->title }}</h3>
                                    <p class="text-sm text-slate-500">{{ $otherJob->created_at->diffForHumans() }} · ${{ number_format($otherJob->salary) }}</p>
                                </div>
