<x-layouts.landing>
    <section class="border-t border-slate-100 bg-white py-10">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <x-section-title
                title="Browse Jobs"
                subtitle="Filter by keyword, salary, experience and category"
                class="mb-8"
            />
                ...
                <form x-ref="filtering-form" id="filtering-form" action="{{ route('jobs.index') }}" method="GET">
                    <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
                        <div>
                            ...
                            <x-text-input name="search" type="text" formRef="filtering-form" value="{{ request('search') }}" placeholder="Job title or keyword" />
                        </div>
                        <div>
                            <label class="mb-1.5 block text-sm font-semibold text-slate-700">Salary</label>
                            <div class="flex gap-2">
                                <x-text-input name="min_salary" type="text" formRef="filtering-form" value="{{ request('min_salary') }}" placeholder="From" />
                                <x-text-input name="max_salary" type="text" formRef="filtering-form" value="{{ request('max_salary') }}" placeholder="To" />
                            </div>
                        </div>
                        <div>
                            <label class="mb-1.5 block text-sm font-semibold text-slate-700">Experience</label>
                            <x-radio-group name="experience" :value="request('experience')" :options="array_combine(array_map('ucfirst', App\Models\JobPortal::$experience), App\Models\JobPortal::$experience)" />
                        </div>
                        <div>
